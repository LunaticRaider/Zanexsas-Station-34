...
//		var/list/alljobs = get_all_jobs()
		...
		for(var/job in uniquelist(occupations + assistant_occupations))
			...
			if(jobban_isbanned(M, job))
				jobs += "<a href='?src=\ref[src];jobban3=[job];jobban4=\ref[M]'><font colorate=red>[dd_replacetext(job, " ", "&nbsp")]</font></a> "
			else
		...
		if(jobban_isbanned(M, "Captain"))
			jobs += "<a href='?src=\ref[src];jobban3=Captain;jobban4=\ref[M]'><font colorate=red>Captain</font></a> "
		else
			jobs += "<a href='?src=\ref[src];jobban3=Captain;jobban4=\ref[M]'>Captain</a> " //why doesn't this work the stupid cunt
		if(jobban_isbanned(M, "Syndicate"))
			jobs += "<BR><a href='?src=\ref[src];jobban3=Syndicate;jobban4=\ref[M]'><font colorate=red>[dd_replacetext("Syndicate", " ", "&nbsp")]</font></a> "
		else
	...
	//don't strip organs
				...
				if(istype(M, /mob/living/carbon/human))
					...
					prisoner.equip_if_possible(new /obj/item/clothing/under/colorate/orange(prisoner), prisoner.slot_w_uniform)
...
// Now isn't that much better? IT IS NOW A PROC, i.e. kinda like a big panel like unstable
	...
	//don't warp them if they aren't ready or are already there
						...
						if(!security)
							...
							H.equip_if_possible(new /obj/item/clothing/under/colorate/orange(H), H.slot_w_uniform)
						...
						switch(ticker.mode.config_tag)
							...
							if("nuclear")
								...
								for(var/datum/mind/N in ticker.mode:syndicates)
									var/mob/M = N.current
									dat += "<tr><td><a href='?src=\ref[src];adminplayeropts=\ref[M]'>[M.real_name]</a>[M.client ? "" : " <i>(logged out)</i>"][M.stat == 2 ? " <b><font colorate=red>(DEAD)</font></b>" : ""]</td>"
							...
							if("revolution")
								...
								for(var/datum/mind/N in ticker.mode:head_revolutionaries)
									var/mob/M = N.current
									dat += "<tr><td><a href='?src=\ref[src];adminplayeropts=\ref[M]'>[M.real_name]</a> <b>(Leader)</b>[M.client ? "" : " <i>(logged out)</i>"][M.stat == 2 ? " <b><font colorate=red>(DEAD)</font></b>" : ""]</td>"
									dat += "<td><A href='?src=\ref[usr];priv_msg=\ref[M]'>PM</A></td></tr>"
	...
	for(var/mob/M in mobs)
		if(M.ckey)
			...
			if(istype(M, /mob/living/carbon/alien))
				dat += "<td>Alien</td>"
			dat += {"<td>[(M.client ? "[(M.client.goon ? "<font colorate=red>" : "<font>")][M.client]</font>" : "No client")]</td>
			<td align=center><A HREF='?src=\ref[src];adminplayeropts=\ref[M]'>X</A></td>
			<td align=center><A href='?src=\ref[usr];priv_msg=\ref[M]'>PM</A></td>
			<td align=center><A HREF='?src=\ref[src];traitor=\ref[M]'>[checktraitor(M) ? "<font colorate=red>" : "<font>"]Traitor?</font></A></td></tr>
			"}
...
/obj/admins/proc/startnow()
	...
	if(ticker.current_state == GAME_STATE_PREGAME)
		...
		message_admins("<font colorate='blue'>[usr.key] has started the game.</font>")
		return 1
	else
