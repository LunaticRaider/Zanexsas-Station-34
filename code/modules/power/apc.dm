...
/proc/RandomAPCWires()
	...
	APCIndexToWirecolorate = list(0, 0, 0, 0)
	APCWirecolorateToIndex = list(0, 0, 0, 0)
	var/flagIndex = 1
	for (var/flag=1, flag<16, flag+=flag)
		var/valid = 0
		while (!valid)
			var/colorateIndex = rand(1, 4)
			if (apcwires[colorateIndex]==0)
				valid = 1
				apcwires[colorateIndex] = flag
				APCIndexToFlag[flagIndex] = flag
				APCIndexToWirecolorate[flagIndex] = colorateIndex
				APCWirecolorateToIndex[colorateIndex] = flagIndex
		flagIndex+=1
	return apcwires
...
/obj/machinery/power/apc/proc/interact(mob/user)
	...
	if(wiresexposed && (!istype(user, /mob/living/silicon)))
		...
		for(var/wiredesc in apcwires)
			var/is_uncut = src.apcwires & APCWirecolorateToFlag[apcwires[wiredesc]]
			t1 += "[wiredesc] wire: "
			if(!is_uncut)
	...
	if(locked && (!istype(user, /mob/living/silicon)))
		...
		t += "External power : <B>[ main_status ? (main_status ==2 ? "<FONT colorate=#004000>Good</FONT>" : "<FONT colorate=#D09000>Low</FONT>") : "<FONT colorate=#F00000>None</FONT>"]</B><BR>"
		t += "Power cell: <B>[cell ? "[round(cell.percent())]%" : "<FONT colorate=red>Not connected.</FONT>"]</B>"
		if(cell)
		...
		if(cell)
			...
			t += "Power cell: <B><FONT colorate=red>Not connected.</FONT></B>"
...
/obj/machinery/power/apc/proc/isWirecolorateCut(var/wirecolorate)
	var/wireFlag = APCWirecolorateToFlag[wirecolorate]
...
/obj/machinery/power/apc/proc/cut(var/wirecolorate)
	var/wireFlag = APCWirecolorateToFlag[wirecolorate]
	var/wireIndex = APCWirecolorateToIndex[wirecolorate]
	apcwires &= ~wireFlag
	switch(wireIndex)
...
/obj/machinery/power/apc/proc/mend(var/wirecolorate)
	var/wireFlag = APCWirecolorateToFlag[wirecolorate]
	var/wireIndex = APCWirecolorateToIndex[wirecolorate] //not used in this function
	apcwires |= wireFlag
	switch(wireIndex)
		if(APC_WIRE_MAIN_POWER1)
...
/obj/machinery/power/apc/proc/pulse(var/wirecolorate)
	//var/wireFlag = apcWirecolorateToFlag[wirecolorate] //not used in this function
	var/wireIndex = APCWirecolorateToIndex[wirecolorate]
	switch(wireIndex)
