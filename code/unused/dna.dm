...
/obj/machinery/scan_console/attack_hand(user as mob)
	...
	if (src.temp)
		...
		if (src.connected)
			...
			dat = "<font colorate='blue'><B>Occupant Statistics:</B></FONT><BR>"
			if (occupant)
				var/t1
				switch(occupant.stat)
					...
					if(1)
						...
						t1 = "*dead*"
				dat += text("[]\tHealth %: [] ([])</FONT><BR><BR>", (occupant.health > 50 ? "<font colorate='blue'>" : "<font colorate='red'>"), occupant.health, t1)
			else
