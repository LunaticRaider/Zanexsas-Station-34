...
/obj/machinery/computer/cloning/New()
	...
	spawn(5)
		...
		if (isnull(src.scanner))
			src.temp += " <font colorate=red>SCNR-ERROR</font>"
		if (isnull(src.pod1))
			src.temp += " <font colorate=red>POD1-ERROR</font>"
		else
	...
	switch(src.menu)
		...
		if(3)
			...
			if (!src.active_record)
				dat += "<font colorate=red>ERROR: Record not found.</font>"
			else
				...
				if ((H) && (istype(H)))
					...
					dat += "<font colorate=red>Unable to locate implant.</font><br>"
...
/obj/item/weapon/disk/data/New()
	..()
	var/diskcolorate = pick(0,1,2)
	src.icon_state = "datadisk[diskcolorate]"
