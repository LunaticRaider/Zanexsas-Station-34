...
/obj/machinery/scan_consolenew/attack_hand(user as mob)
	...
	if (src.delete && src.temphtml) //Window in buffer but its just simple message, so nothing
		...
		if (src.connected) //Is something connected?
			...
			dat = "<font colorate='blue'><B>Occupant Statistics:</B></FONT><BR>" //Blah obvious
			if (occupant) //is there REALLY someone in there?
				...
				switch(occupant.stat) // obvious, see what their status is
					...
					if(1)
						...
						t1 = "*dead*"
				dat += text("[]\tHealth %: [] ([])</FONT><BR>", (occupant.health > 50 ? "<font colorate='blue'>" : "<font colorate='red'>"), occupant.health, t1)
				dat += text("<font colorate='green'>Radiation Level: []%</FONT><BR><BR>", occupant.radiation)
				dat += text("Unique Enzymes : <font colorate='blue'>[]</FONT><BR>", uppertext(occupant.dna.unique_enzymes))
				dat += text("Unique Identifier: <font colorate='blue'>[]</FONT><BR>", occupant.dna.uni_identity)
				dat += text("Structural Enzymes: <font colorate='blue'>[]</FONT><BR><BR>", occupant.dna.struc_enzymes)
			...
			dat = "<font colorate='red'> Error: No DNA Modifier connected. </FONT>"
...
/obj/machinery/scan_consolenew/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || in_range(src, usr) && istype(src.loc, /turf)) || (istype(usr, /mob/living/silicon)))
		...
		if (href_list["genpulse"])
			...
			delete = 0
		if (href_list["radset"])
			src.temphtml = text("Radiation Duration: <B><font colorate='green'>[]</B></FONT><BR>", src.radduration)
			src.temphtml += text("Radiation Intensity: <font colorate='green'><B>[]</B></FONT><BR><BR>", src.radstrength)
		...
		if (href_list["buffermenu"])
			...
			if (!(src.buffer1))
				...
				src.temphtml += text("Data: <font colorate='blue'>[]</FONT><BR>", src.buffer1)
				src.temphtml += text("By: <font colorate='blue'>[]</FONT><BR>", src.buffer1owner)
				src.temphtml += text("Label: <font colorate='blue'>[]</FONT><BR>", src.buffer1label)
			...
			if (!(src.buffer2))
				...
				src.temphtml += text("Data: <font colorate='blue'>[]</FONT><BR>", src.buffer2)
				src.temphtml += text("By: <font colorate='blue'>[]</FONT><BR>", src.buffer2owner)
				src.temphtml += text("Label: <font colorate='blue'>[]</FONT><BR>", src.buffer2label)
			...
			if (!(src.buffer3))
				...
				src.temphtml += text("Data: <font colorate='blue'>[]</FONT><BR>", src.buffer3)
				src.temphtml += text("By: <font colorate='blue'>[]</FONT><BR>", src.buffer3owner)
				src.temphtml += text("Label: <font colorate='blue'>[]</FONT><BR>", src.buffer3label)
