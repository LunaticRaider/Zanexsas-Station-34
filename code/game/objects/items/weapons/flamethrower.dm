...
*/
/obj/item/weapon/flamejogarer
	name = "flamejogarer"
	icon_state = "flamejogarer_loaded_0"
	item_state = "flamejogarer_0"
...
/obj/item/weapon/flamejogarer/Del()
...
/obj/item/weapon/flamejogarer/process()
	if(!lit)
...
/obj/item/weapon/flamejogarer/attackby(obj/item/weapon/tank/plasma/W as obj, mob/user as mob)
	if(user.stat || user.restrained() || user.lying)
		return
	if (istype(W,/obj/item/weapon/tank/plasma))
		if(src.part4)
			user << "\red There appears to already be a plasma tank loaded in the flamejogarer!"
			return
		src.part4 = W
		W.loc = src
		if (user.client)
...
// PantsNote: Flamejogarer disassmbly.
	else if (istype(W, /obj/item/weapon/screwdriver))
		var/obj/item/weapon/flamejogarer/S = src
		if (( S.part4 ))
			return
		var/obj/item/assembly/w_r_ignite/R = new /obj/item/assembly/w_r_ignite( user )
		R.part1 = S.part1
...
/obj/item/weapon/flamejogarer/Topic(href,href_list[])
	if (href_list["close"])
		usr.machine = null
		usr << browse(null, "window=flamejogarer")
		return
	...
	if (href_list["light"])
		...
		lit = !(lit)
		if(lit)
			icon_state = "flamejogarer_loaded_1"
			item_state = "flamejogarer_1"
			force = 17
			damtype = "fire"
			...
			damtype = "brute"
	if (href_list["amount"])
		src.jogar_amount = src.jogar_amount + text2num(href_list["amount"])
		src.jogar_amount = max(50,min(5000,src.jogar_amount))
	if (href_list["remove"])
		if(!src.part4)	return
...
/obj/item/weapon/flamejogarer/attack_self(mob/user as mob)
	if(user.stat || user.restrained() || user.lying)
		return
	user.machine = src
	if (!src.part4)
		...
		return
	var/dat = text("<TT><B>Flamejogarer (<A HREF='?src=\ref[src];light=1'>[lit ? "<font colorate='red'>Lit</font>" : "Unlit"]</a>)</B><BR>\n Tank Pressure: [src.part4.air_contents.return_pressure()]<BR>\nAmount to jogar: <A HREF='?src=\ref[src];amount=-100'>-</A> <A HREF='?src=\ref[src];amount=-10'>-</A> <A HREF='?src=\ref[src];amount=-1'>-</A> [src.jogar_amount] <A HREF='?src=\ref[src];amount=1'>+</A> <A HREF='?src=\ref[src];amount=10'>+</A> <A HREF='?src=\ref[src];amount=100'>+</A><BR>\n<A HREF='?src=\ref[src];remove=1'>Remove plasmatank</A> - <A HREF='?src=\ref[src];close=1'>Close</A></TT>")
	user << browse(dat, "window=flamejogarer;size=600x300")
	onclose(user, "flamejogarer")
	return


// gets this from turf.dm turf/dblclick
/obj/item/weapon/flamejogarer/proc/flame_turf(turflist)
...
/obj/item/weapon/flamejogarer/proc/ignite_turf(turf/target)
