...
/obj/machinery/alarm/examine()
	...
	if (get_dist(usr, src) <= 3 || istype(usr, /mob/living/silicon/ai))
		...
		t1 = round(t1, 0.0010)
		if ((60 < t1 && t1 < 80))
			t += text("<font colorate=blue>Nitrogen: []</font> ", t1)
		else
			t += text("<font colorate=red>Nitrogen: []</font> ", t1)
		t1 = T.oxygen / turf_total * 100
		t1 = round(t1, 0.0010)
		if ((20 < t1 && t1 < 24))
			t += text("<font colorate=blue>Oxygen: []</font> ", t1)
		else
			t += text("<font colorate=red>Oxygen: []</font> ", t1)
		t1 = T.poison / turf_total * 100
		t1 = round(t1, 0.0010)
		if (t1 < 0.5)
			t += text("<font colorate=blue>Plasma: []</font> ", t1)
		else
			t += text("<font colorate=red>Plasma: []</font> ", t1)
		t1 = T.co2 / turf_total * 100
		t1 = round(t1, 0.0010)
		if (t1 < 1)
			t += text("<font colorate=blue>CO2: []</font> ", t1)
		else
			t += text("<font colorate=red>CO2: []</font> ", t1)
		t1 = T.sl_gas / turf_total * 100
		t1 = round(t1, 0.0010)
		if (t1 < 5)
			t += text("<font colorate=blue>NO2: []</font>", t1)
		else
			t += text("<font colorate=red>NO2: []</font>", t1)
		t1 = T.temp - T0C
		if (T.temp > 326.444 || T.temp < 282.591)
			t += text("<br><font colorate=red>Temperature: []</font>", t1)
		else
			t += text("<br><font colorate=blue>Temperature: []</font>", t1)
