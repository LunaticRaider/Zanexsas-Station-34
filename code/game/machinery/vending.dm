...
/obj/machinery/vending/proc/build_inventory(var/list/path_list,var/list/amt_list,hidden=0)
	...
	for(var/p=1, p <= path_list.len ,p++)
		...
		R.display_colorate = pick("red","blue","green")
...
/obj/machinery/vending/attack_hand(mob/user as mob)
	...
	if(panel_open)
		...
		for(var/wiredesc in vendwires)
			var/is_uncut = src.wires & APCWirecolorateToFlag[vendwires[wiredesc]]
			pdat += "[wiredesc] wire: "
			if(!is_uncut)
	...
	if (src.product_records.len == 0)
		dat += "<font colorate = 'red'>No product loaded!</font>"
	else
		...
		for (var/datum/data/vending_product/R in display_records)
			dat += "<FONT colorate = '[R.display_colorate]'><B>[R.product_name]</B>:"
			dat += " [R.amount] </font>"
			if (R.amount > 0)
				dat += "<a href='byond://?src=\ref[src];vend=\ref[R]'>Vend</A>"
			else
				dat += "<font colorate = 'red'>SOLD OUT</font>"
			dat += "<br>"
	...
	if ((usr.contents.Find(src) || (in_range(src, usr) && istype(src.loc, /turf))))
		...
		if ((href_list["vend"]) && (src.vend_ready))
			...
			if (!( istype(usr.equipped(), /obj/item/weapon/wirecutters) ))
				...
				return
			if (src.isWirecolorateCut(twire))
				src.mend(twire)
			else
				src.cut(twire)
...
/obj/machinery/vending/proc/jogar_item()
	var/obj/jogar_item = null
	...
	for(var/datum/data/vending_product/R in src.product_records)
		...
		R.amount--
		jogar_item = new dump_path(src.loc)
		break

	spawn(0)
		jogar_item.jogar_at(target, 16, 3)
	src.visible_message("\red <b>[src] launches [jogar_item.name] at [target.name]!</b>")
	return 1

/obj/machinery/vending/proc/isWirecolorateCut(var/wirecolorate)
...
/obj/machinery/vending/proc/cut(var/wirecolorate)
	var/wireFlag = APCWirecolorateToFlag[wirecolorate]
...
/obj/machinery/vending/proc/mend(var/wirecolorate)
	var/wireFlag = APCWirecolorateToFlag[wirecolorate]
...
/obj/machinery/vending/proc/pulse(var/wirecolorate)
	var/wireIndex = APCWirecolorateToIndex[wirecolorate]
	switch(wireIndex)
		if(WIRE_EXTEND)
