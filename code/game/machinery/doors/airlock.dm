...
/*
	...
	isWirecolorateCut - returns 1 if that colorate wire is cut, or 0 if not
...
/proc/RandomAirlockWires()
	...
	airlockIndexToWirecolorate = list(0, 0, 0, 0, 0, 0, 0, 0, 0)
	airlockWirecolorateToIndex = list(0, 0, 0, 0, 0, 0, 0, 0, 0)
	var/flagIndex = 1
	for (var/flag=1, flag<512, flag+=flag)
		var/valid = 0
		while (!valid)
			var/colorateIndex = rand(1, 9)
			if (wires[colorateIndex]==0)
				valid = 1
				wires[colorateIndex] = flag
				airlockIndexToFlag[flagIndex] = flag
				airlockIndexToWirecolorate[flagIndex] = colorateIndex
				airlockWirecolorateToIndex[colorateIndex] = flagIndex
		flagIndex+=1
	return wires

/* Example:
Airlock wires colorate -> flag are { 64, 128, 256, 2, 16, 4, 8, 32, 1 }.
Airlock wires colorate -> index are { 7, 8, 9, 2, 5, 3, 4, 6, 1 }.
Airlock index -> flag are { 1, 2, 4, 8, 16, 32, 64, 128, 256 }.
Airlock index -> wire colorate are { 9, 4, 6, 7, 5, 8, 1, 2, 3 }.
*/
...
/obj/machinery/door/airlock/proc/pulse(var/wirecolorate)
	//var/wireFlag = airlockWirecolorateToFlag[wirecolorate] //not used in this function
	var/wireIndex = airlockWirecolorateToIndex[wirecolorate]
	switch(wireIndex)
...
/obj/machinery/door/airlock/proc/cut(var/wirecolorate)
	var/wireFlag = airlockWirecolorateToFlag[wirecolorate]
	var/wireIndex = airlockWirecolorateToIndex[wirecolorate]
	wires &= ~wireFlag
	switch(wireIndex)
		if(AIRLOCK_WIRE_MAIN_POWER1 || AIRLOCK_WIRE_MAIN_POWER2)
...
/obj/machinery/door/airlock/proc/mend(var/wirecolorate)
	var/wireFlag = airlockWirecolorateToFlag[wirecolorate]
	var/wireIndex = airlockWirecolorateToIndex[wirecolorate] //not used in this function
	wires |= wireFlag
	switch(wireIndex)
		if(AIRLOCK_WIRE_MAIN_POWER1 || AIRLOCK_WIRE_MAIN_POWER2)
...
/obj/machinery/door/airlock/proc/isWirecolorateCut(var/wirecolorate)
	var/wireFlag = airlockWirecolorateToFlag[wirecolorate]
...
/obj/machinery/door/airlock/animater(animation)
	switch(animation)
		if("opening")
			if(overlays) overlays = null
			if(p_open)
...
/obj/machinery/door/airlock/attack_hand(mob/user as mob)
	...
	if (src.p_open)
		...
		//t1 += text("[]: ", airlockFeatureNames[airlockWirecolorateToIndex[9]])
		var/list/wires = list(
			"Orange" = 1,
			"Dark red" = 2,
			"White" = 3,
		...
		for(var/wiredesc in wires)
			var/is_uncut = src.wires & airlockWirecolorateToFlag[wires[wiredesc]]
			t1 += "[wiredesc] wire: "
			if(!is_uncut)
		...
		if ((in_range(src, usr) && istype(src.loc, /turf)))
			...
			if (href_list["wires"])
				...
				if(!istype(usr.equipped(), /obj/item/device/radio/signaler))
					...
					return
				if(src.isWirecolorateCut(wirenum))
