		...
		if (src.authenticated && src.modify)
			...
			for(var/A in get_all_accesses())
				if(A in src.modify.access)
					accesses += "<a href='?src=\ref[src];access=[A];allowed=0'><font colorate=\"red\">[dd_replacetext(get_access_desc(A), " ", "&nbsp")]</font></a> "
				else
...
/obj/machinery/mass_driver/proc/drive(amount)
	...
	for(var/atom/movable/O in src.loc)
		if(!O.anchored)
			...
			use_power(500)
			spawn( 0 )
				O.jogar_at(target, drive_range * src.power, src.power)
