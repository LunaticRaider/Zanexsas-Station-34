	...
	attack_hand(mob/user as mob)
		...
		if(src.occupant)
			if(src.occupant.health <= -100)
				health_text = "<FONT colorate=red>Dead</FONT>"
			else if(src.occupant.health < 0)
				health_text = "<FONT colorate=red>[src.occupant.health]</FONT>"
			else
				health_text = "[src.occupant.health]"
		if(air_contents.temperature > T0C)
			temp_text = "<FONT colorate=red>[air_contents.temperature]</FONT>"
		else if(air_contents.temperature > 225)
			temp_text = "<FONT colorate=black>[air_contents.temperature]</FONT>"
		else
			temp_text = "<FONT colorate=blue>[air_contents.temperature]</FONT>"
		if(src.beaker)
			beaker_text = "<B>Beaker:</B> <A href='?src=\ref[src];eject=1'>Eject</A>"
		else
			beaker_text = "<B>Beaker:</B> <FONT colorate=red>No beaker loaded</FONT>"
		var/dat = {"<B>Cryo cell control system</B><BR>
			...
			[beaker_text]<BR><BR>
			<B>Current occupant:</B> [src.occupant ? "<BR>Name: [src.occupant]<BR>Health: [health_text]<BR>Oxygen deprivation: [src.occupant.oxyloss]<BR>Brute damage: [src.occupant.bruteloss]<BR>Fire damage: [src.occupant.fireloss]<BR>Toxin damage: [src.occupant.toxloss]<BR>Body temperature: [src.occupant.bodytemperature]" : "<FONT colorate=red>None</FONT>"]<BR>
