				...
				if (!src.target.handcuffed && !src.arrest_type)
					...
					spawn(60)
						if (get_dist(src, src.target) <= 1)
							...
							playsound(src, pick('bgod.ogg', 'biamthelaw.ogg', 'bsecureday.ogg', 'bradio.ogg', 'binsult.ogg', 'bcreep.ogg'), 50, 0)
	...
	proc/look_for_perp()
		...
		for (var/mob/living/carbon/C in view(7,src)) //Let's find us a criminal
			...
			if (!src.threatlevel)
				...
				playsound(src, pick('bcriminal.ogg', 'bjustice.ogg', 'bfreeze.ogg'), 50, 0)
